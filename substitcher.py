...
def wanted(str):
    return wanted_num(int(str[-8:-4]))

def wanted_num(i):
    return i%16 >= 4 and i%16 <= 6 and i/16 >= 39 and i/16 <= 43.5
...
def stitcher(image_path, n_rows, save_name, master=None):
    ...
    imagepaths = [os.path.join(image_path, i) for i in images if wanted(i)]
    ...
    with open(os.path.join(image_path, "imagelist.txt"), 'w') as imagelist:
        ...
        if len(imagelist) < 1:
            notify("Images not found in "+image_path)
            print("Image stitching found no images")
            return False
