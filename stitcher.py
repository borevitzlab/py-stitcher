    ...
    if start < date < end:
        ...
        if stitcher(path, n_rows, save_path, master):
            ...
            if log:
                ...
                data_writer.writerow([year, month, day, hour, path, save_path, master, "success"])
        else:
            notify("Failure for {}: {}/{}/{} {}:00".format(name, day, month, year, hour))
            if log:
                with open(log, 'a') as csvfile:
                    data_writer = csv.writer(csvfile, delimiter=' ', quotechar='|', quoting=csv.QUOTE_MINIMAL)
                data_writer.writerow([year, month, day, hour, path, save_path, master, "failure"])
...
def stitch_month(path, n_rows, save_directory, name, year, month, start, end, master, log):
    ...
    if master:
        ...
        stitcher(master_src, n_rows, os.path.join(path, "master"))
